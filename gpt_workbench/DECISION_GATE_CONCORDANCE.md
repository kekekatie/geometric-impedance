...
| **G4** shuffle-kill | killed by the sealed stratified shuffle? | `R_kill` = **paired** `red_{c,o}=(plain−shuf)/plain` aggregated by `M₉` | — | `R_kill ≥ 0.70` | **any** required fold/config `plain ≤ δ_cap` or `≤0` → `red` undefined → **global G4 undefined → mixed** (never recomputed over a surviving subset) | **`M₉`** (paired) |
...
requires **(classical G1) ∧ G5**; **a G5 failure does not erase the coherent transport result**, it
only withholds the modifier. **G7** descriptive; **G8** secondary. Any undefined-denominator route →
**mixed/undetectable** (G4: any undefined required `red_{c,o}` → global G4 undefined → mixed, never
recomputed over a surviving subset). A G1 coherent failure downgrades the global claim (it never
recomputes `M₉`). Physical-radius outcomes (compression / representational / **survives-stress-controls**
/ mixed / infeasible) use the same statistics: compression from `M₉,address` at r=2 vs r=16 with
`δ_cap`+`ρ*`; "compatible with representation collapse" from a small `Δ_ap` (G7, **qualitative — no
threshold, no comparison to `δ_cap`**); "survives the frozen stress controls" (replaces "stable
residual") from G3 + G6 + G2(`M_perm,7`), with G7 reported descriptively — **no categorical
"irreducible" verdict.** `M₉` always spans the nine configs; `M_perm,7` the seven feasible; platinum
e16/e18 cannot pass G2.
