...
| [`seed_dynamics/`](seed_dynamics/) | **the seed rule running (first resonance dynamics).** Twin seeds fill gaps alongside road growth; controls: no seeds, dust, fake twins. **Any seeding halves the gaps (30.6 → ~15–20), but real likeness adds nothing** (twin ≈ fake at r=4,6; fakes better at r=2), no lock-in. Reason: in a fixed tiling the shape is already decided, so a seed has nothing to teach. **Next: grow the tiling itself, where growth can go wrong** (cf. Onoda–Steinhardt–DiVincenzo–Socolar 1988). |
| [`laying_the_street/`](laying_the_street/) | **Wallace and Gromit: lay the Fibonacci street letter by letter, with no map** (only letters already laid). Local rules always defect (as 1-D theory requires); **same-scale twin copying → repetition (32/32 seeds: resonant lock-in)**; anti-copy / dice → noise; **consulting the street's own zoomed-out self → never repeats, perfect from 2 seeds, mistakes kept as scars (14) or inflating defects (15), never healed, never lost**; a wrong zoom or a zoom read at random fails. *Likeness across scales, not at the same scale, lets a quasicrystal build itself.* |
| [`laying_the_tiling/`](laying_the_tiling/) | **the 2-D sequel: Penrose laid half-tile by half-tile, no map** (only a learned atlas of the 8 vertex types). Local + dice **jams 30/30**; local *forced-first* never jams (500 tiles) but needs 3–4 guesses and **builds a different Penrose universe** each time — and the universes **differ along thin bands running exactly along ribbon directions** (tested: 11–12× elongated vs blobs ≤2.6; pre-registered 'one shared direction' prediction failed, recorded); same-scale COPY lays an illegal tile; **SCALE (own zoomed-out self) lays the self-similar tiling exactly, zero guesses** — but only from a truly self-similar centre. |
