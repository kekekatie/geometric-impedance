...
| [`laying_the_tiling/`](laying_the_tiling/) | **the 2-D sequel: Penrose laid half-tile by half-tile, no map** (only a learned atlas of the 8 vertex types). Local + dice **jams 30/30**; local *forced-first* never jams (500 tiles) but needs 3–4 guesses and **builds a different Penrose universe** each time (differences lie along ribbon-like bands); same-scale COPY lays an illegal tile; **SCALE (own zoomed-out self) lays the self-similar tiling exactly, zero guesses** — but only from a truly self-similar centre. |
